...
| `merge_maps.py` | 把各图/各源数据集合并成统一训练集：逐源校验 `names`==主表（`--master`，默认 `data/wgc_review/dataset.yaml`；不一致则拒绝除非 `--map-source` 重映射，防类别错位）、内容哈希去重（去重复帧）、按类保证 val 覆盖稀有类的重切 train/val、写不含 `path` 的 dataset.yaml（本地/AutoDL 通用）。`--dry-run` 只打印计数/直方图/覆盖情况 |
| `sample_frames.py` | 视频帧信息增益采样：逐视频对 auto_label 输出做贪心筛选，去掉冗余相邻帧（avg-hash 指纹位距≥`--min-hamming` 才留），并**强制保留杂物得分 top `--clutter-top` 比例的难帧**（杂物分=该帧待复核框数，掉落物品多⇒低置信检测堆叠⇒分高，正是识别准确率洼地）。把选中帧的 image/label/review 拷到 `--out` 供 export-review→vlm-review→apply-review→review_pack 复核链 |
...
- 用 ruff 检查代码风格（`uvx ruff check tools/yolo_data.py tools/merge_maps.py tools/sample_frames.py`）。
- `cv2.imwrite` 在 Windows 对**含中文（非 ASCII）路径静默失败**（返回 False 不抛错，文件不落盘）——数据集/裁剪输出目录一律用拼音（如 `data/maps/sheshou_xunlianchang`），中文只保留在 json/dataset.yaml。`sample_frames.py` 读图用 `_imread`（`np.fromfile`+`imdecode` 兜底）同理防中文路径。
- 新增子命令时在 `main()` 用 `add_parser` 注册，并给函数加 `args` 参数、保持 argparse 风格。
- `merge_maps.py` 的输出 dataset.yaml 故意**不含 `path` 键**（本地与 AutoDL 实例均以 yaml 目录为根），可直接上传 AutoDL 训练（`docs/autodl_training.md`）。类别表不一致的源默认被拒绝（防静默错位），需人工提供 `--map-source` 映射表才可并入。
